...
/*
	if (crypto_aead_aes256gcm_is_available() == 0) {
		throw std::runtime_error("encAES256GCM_sodium(): AES-NI not available on this system"); // 縺昴ｂ縺昴ｂ蛻ｩ逕ｨ荳榊庄
	}
*/
	crypto_aead_aes256gcm_encrypt_detached(
		result.cipher.data(), result.tag.data(), nullptr,
		plaintext.data(), plaintext.size(),
		aad.data(), aad.size(),  // 竊舌％縺薙〒 AAD 繧呈ｸ｡縺・
...
/*
	if (crypto_aead_aes256gcm_is_available() == 0) {
		throw std::runtime_error("decAES256GCM_sodium(): AES-NI not available on this system");
	}
*/
